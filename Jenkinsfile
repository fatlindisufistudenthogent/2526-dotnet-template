pipeline {
    ...
    stages {
        ...
        stage('Deploy') {
            steps {
                sh '''
                    sh "rsync -av --delete --no-perms --no-owner --no-times -e 'ssh -i /var/lib/jenkins/.ssh/id_rsa -o StrictHostKeyChecking=no -o IdentitiesOnly=yes -F /dev/null' ./publish/ vagrant@192.168.56.10:/opt/rise/"
